module core_top(
    ...
    assign if_inst = rst ? 32'h02800000 :
                     (id_br_taken || br_need_cancel) ? 32'h02800000 :
                     inst_sram_data_ok ? inst_sram_rdata : inst_sram_rdata_safe;
   ...
   // wire id_cancel;   //跳转的话，需要置�???????1
    ...
    assign pc_br_taken=id_br_taken|(wb_ex===1'b1)|(wb_is_ertn==1'b1)|(id_is_ertn==1'b1);
    //assign pc_br_target=id_br_target|({32{wb_ex===1'b1}}&csr_rvalue)|({32{wb_is_ertn===1'b1}}&csr_era_pc);
    assign pc_br_target =    (wb_is_ertn===1'b1 || id_is_ertn===1'b1)  ?   csr_era_pc  :
...
`endif
    ...
    assign id_allow_in = exe1_allow_in && !(id_ready_go===1'b0) && !exe1_dropped;
    ...
    // DEBUG: ERTN trace
    always @(posedge clk) begin
        if (!rst && wb_is_ertn) begin
            $display("[ERTN] %0t: wb_pc=%h era=%h wb_ex=%b pc_br_taken=%b pc_br_target=%h trap_flush=%b pipe_stall=%b",
                     $time, wb_pc, csr_era_pc, wb_ex, pc_br_taken, pc_br_target, trap_flush, ~pipline_is_not_stalled);
        end
    end
...
`endif
    ...
    assign id_need_cancel = (id_br_taken || br_need_cancel) ? 2'b10 : id_need_cancel_raw;
