...
// ============== div control signal generation (moved from exe_staller.v)
// alu_op[24] = inst_div
// alu_op[25] = inst_modw
// alu_op[26] = inst_divu
...
// div_complete is already declared as wire at the top, change to reg
reg div_complete_reg;
assign div_complete = div_complete_reg;

// division completion status register
always @(posedge clk) begin
    if (reset) begin
        div_complete_reg <= 1'b0;
    end
    else if (es_allowin && is_div_op) begin
        // new division instruction enters exe stage, reset completion flag
        div_complete_reg <= 1'b0;
    end
    else if ((alu_op[24] || alu_op[25]) && ready_s) begin
        // signed division complete
        div_complete_reg <= 1'b1;
    end
    else if ((alu_op[26] || alu_op[27]) && ready_u) begin
        // unsigned division complete
        div_complete_reg <= 1'b1;
    end
end

// exe stage ready_go signal: if division instruction, wait for completion; otherwise directly 1
assign es_ready_go = (es_valid && is_div_op) ? div_complete : 1'b1;

// exe stage allowin signal
assign es_allowin = !es_valid || (es_ready_go && ms_allowin);

// exe stage to mem stage valid signal
